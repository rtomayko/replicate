module GitHub
  module Replicator
    # Replicator filter that writes info to the console.
    class Status
      def initialize(dumper, out, verbose=false, quiet=false)
        @dumper = dumper
        @out = out
        @verbose = verbose
        @quiet = quiet
      ...
      def verbose_log(type, id, attrs, object)
        ...
        @out.puts "dump: %-30s %s" % [type.sub('GitHub::Replicator::', ''), desc]
      end

      def normal_log(type, id, attrs, object)
        @out.write "==> dump: #{@count} objects      \r"
      end
      ...
      def dump_stats(stats=@dumper.stats.dup)
        @out.puts "==> dump #{@count} objects complete:"
        width = 0
